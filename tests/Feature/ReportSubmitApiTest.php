...
class ReportSubmitApiTest extends TestCase
{
  ...
  public function _日誌投稿が無事に行えるAPI()
  {
    ...
    $response = $this->actingAs($this->user)
        ->json('POST', route('report.create'), $data);
    ...
    //ID桁数が16桁であること
    $this->assertRegExp('/^[0-9a-zA-Z-_]{16}$/', $report->id);
  ...
  //  * @test
  //  */
  // public function _DBエラーの場合、日誌投稿しないこと()
  // {
  //   $this->withoutExceptionHandling();
  //   Schema::drop('reports');
  //   // 仮の入力でーたを用意
  ...
  //       廣い門の下には、この男の外に誰もゐない。唯、所々丹塗の剥げた、大きな圓柱に、蟋蟀が一匹とまつてゐる。羅生門が、朱雀大路にある以上は、この男の外にも、雨やみをする市女笠や揉烏帽子が、もう二三人にんはありさうなものである。それが、この男の外には誰もゐない。',
  //   ];
  //
  //   $response = $this->actingAs($this->user)
  //       ->json('POST', route('report.create'), $data);
